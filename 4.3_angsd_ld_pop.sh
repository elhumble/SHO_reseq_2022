...
TARGET_DIR=/exports/cmvm/eddie/eb/groups/ogden_grp/emily/SHO_reseq_2022/data/out/5_angsd/DS_NS/pop_beagle
...
echo pop $1
ninds=$(cat file_lists/${1}_rmdup_bams_DS_NS_POP.txt | wc -l)
echo $ninds
...
zcat $this_beagle | awk 'NR % 50 == 0' | cut -f 4- | gzip  > ${this_beagle%.beagle.gz}_subsampledbeagle.gz


# Get list of maf files

mafs=$(ls -1 ${TARGET_DIR}/${1}*mafs.gz)
...
this_sub_beagle=$(echo "${this_beagle%.beagle.gz}_subsampledbeagle.gz")
